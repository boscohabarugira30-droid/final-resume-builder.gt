...
export default function SidebarControls({ data, update, colors, themeColor, setThemeColor, setPhoto, photo, setView }: any) {
  return (
    <aside className="w-full lg:w-[450px] h-full bg-white border-r shadow-2xl overflow-y-auto p-8 flex flex-col gap-6 no-print">
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-3">
          <input placeholder="Full Name" value={data.name} onChange={e => update("name", e.target.value)} className="w-full p-3 bg-slate-50 rounded-xl border border-slate-300 text-sm font-bold" />
          <input placeholder="Job Title" value={data.role} onChange={e => update("role", e.target.value)} className="w-full p-3 bg-slate-50 rounded-xl border border-slate-300 text-sm font-semibold" />
          <textarea placeholder="Experience" value={data.experience} onChange={e => update("experience", e.target.value)} className="w-full p-3 bg-slate-50 rounded-xl border border-slate-300 text-xs h-32" />
        </div>
